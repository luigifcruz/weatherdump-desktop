...
class Processor extends Component {
  ...
  componentDidMount() {
    global.client.getManifest(this.datalink).then(manifest => {
      this.props.dispatch(
        rxa.updateManifest(manifest.Parser, manifest.Composer)
      );
    });
  }
              ...
              {Object.entries(this.props.manifestParser).map((p, i) => {
                return (
                  <div
                    key={i}
                    onClick={() =>
                      this.props.dispatch(rxa.toggleParserActivation(p[0]))
                    }
                    className={
                      p[1].Activated ? 'mtx-cell cell-active' : 'mtx-cell'
                    }
                  >
                    {p[1].Name}
                  </div>
              ...
              {Object.entries(this.props.manifestComposer).map((p, i) => {
                return (
                  <div
                    key={i}
                    onClick={() =>
                      this.props.dispatch(rxa.toggleComposerActivation(p[0]))
                    }
                    className={
                      p[1].Activated ? 'mtx-cell cell-active' : 'mtx-cell'
                    }
                ...
                if (!p[0].includes('Export')) {
                  return (
                    <div
                      key={i}
                      onClick={() =>
                        this.props.dispatch(rxa.toggleEnhancement(p[0]))
                      }
                      className={
                        p[1].Activated ? 'mtx-cell cell-active' : 'mtx-cell'
                      }
